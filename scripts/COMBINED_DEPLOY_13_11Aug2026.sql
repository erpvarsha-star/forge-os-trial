-- ============================================================================
-- COMBINED_DEPLOY_13_11Aug2026.sql
--
--   ⚡ RUN THIS ONE FILE. ⚡
--
-- Contains PATCH_13 (photo storage), generated by concatenation so it cannot
-- drift from the source patch.
--
...
-- (RLS recursion fix + RLS hardening) which you confirmed ran successfully.
--
-- WHAT THIS ADDS
--   Storage for 5S and maintenance photos. The camera on those two screens was
--   never wired up — it wrote a hardcoded placeholder.com URL into photo_url,
--   so every photo a supervisor tried to review showed as broken. The app now
--   captures and uploads real photos, which needs a bucket to put them in.
--   There was no Supabase Storage bucket in this project at all.
--
-- Idempotent — safe to run again if you are unsure whether it took.
--
-- NOTE: the app build that uses this ships at the same time. Run this BEFORE
-- installing the new APK, otherwise photo upload will fail with a missing
-- bucket error.
-- ============================================================================
