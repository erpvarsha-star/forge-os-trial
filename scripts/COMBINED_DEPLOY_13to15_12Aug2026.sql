-- ============================================================================
-- COMBINED_DEPLOY_13to15_12Aug2026.sql
--
--   ⚡ RUN THIS ONE FILE. ⚡
--
-- Contains PATCH_13 + PATCH_14 + PATCH_15, generated by concatenation so it
-- cannot drift from the source patches. Supersedes COMBINED_DEPLOY_13to14
-- (deleted; its contents are inside this one). If you already ran that, this
-- is still safe — every statement is idempotent.
--
-- ALREADY APPLIED, do NOT re-run: FINAL_SCHEMA, the seeds, PATCH_01..09,
-- COMBINED_DEPLOY_03to09, PATCH_10 (PIN auth), and COMBINED_DEPLOY_11to12.
--
-- WHAT THIS ADDS
--
--   PATCH_13 — Storage for 5S and maintenance photos. The camera on those two
--   screens wrote a hardcoded placeholder.com URL, so every photo a supervisor
--   tried to review showed as broken. There was no Storage bucket at all.
--
--   PATCH_14 — The Google Forms registry (form_links, 24 daily forms) plus the
--   shift deadline schedule, which the Forms tab and the shift-reminder edge
--   function both read. Also adds notifications.related_entity_type /
--   related_entity_id, whose absence had been silently failing EVERY in-app
--   notification insert from EVERY edge function since day one.
--
--   PATCH_15 — Landing tables for the Operations Dashboard sync:
--   form_submissions (which shifts actually came in, so the Forms tab shows
--   what is outstanding) and production_records (the RAW tab rows, so
--   department production finally appears on the manager and owner
--   dashboards). Filled by syncOpsDashboardToSupabase() in scripts/ALERT.gs.
--
--   Neither PATCH_15 table has an INSERT policy, deliberately: the sync writes
--   with the service role, which bypasses RLS, so nothing running on a phone
--   can forge a submission or a production number.
--
-- AFTER RUNNING THIS, two things in the Apps Script editor:
--   1. Project Settings → Script Properties → add SUPABASE_URL and
--      SUPABASE_SERVICE_ROLE_KEY (the service role key — never paste it in chat)
--   2. Run testSupabaseSync() once to confirm the push works
--
-- Idempotent — safe to run again if you are unsure whether it took.
-- ============================================================================

-- ===== BEGIN PATCH_13_photo_storage_11Aug2026.sql =====
...
-- ===== END PATCH_13_photo_storage_11Aug2026 =====

-- ===== BEGIN PATCH_14_form_registry_12Aug2026.sql =====
...
-- ===== END PATCH_14_form_registry_12Aug2026 =====

-- ===== BEGIN PATCH_15_ops_sync_12Aug2026.sql =====
...
-- ===== END PATCH_15_ops_sync_12Aug2026 =====
