-- ============================================================================
-- COMBINED_DEPLOY_13to14_12Aug2026.sql
--
--   ⚡ RUN THIS ONE FILE. ⚡
--
-- Contains PATCH_13 + PATCH_14, generated by concatenation so it cannot drift
-- from the source patches. Supersedes COMBINED_DEPLOY_13_11Aug2026.sql — if
-- you never ran that one, run this instead; if you did, this is still safe,
...
-- (RLS recursion fix + RLS hardening).
--
-- WHAT THIS ADDS
--
...
--   tried to review showed as broken. There was no Storage bucket in the
--   project at all. Run this BEFORE installing the new APK.
--
--   PATCH_14 — The Google Forms registry, so the app can show it. Adds
--   form_links (24 daily forms across the six shops, from your registry
--   sheet) plus the shift deadline schedule in plant_config, which the new
--   Forms tab and the shift-reminder edge function both read.
--
--   PATCH_14 also fixes a live bug: notifications.related_entity_type and
--   related_entity_id never existed in the deployed schema, but push.ts has
--   always written them — so PostgREST rejected the insert and EVERY in-app
--   notification from EVERY edge function has been failing silently since
--   day one. Both columns are added here.
--
...
-- ===== END PATCH_13 / BEGIN PATCH_14_form_registry_12Aug2026.sql =====
