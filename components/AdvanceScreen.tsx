...
export function AdvanceScreen() {
  ...
  const { requests, isLoading, applyAdvance } = useAdvance(employee?.id || '')
  ...
  const handleApply = async () => {
    ...
    await applyAdvance({ amount: parseFloat(amount), repayment_months: parseInt(repaymentMonths), reason })
      ...
      <Modal visible={showModal} transparent animationType="slide">
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white rounded-t-2xl p-6">
            ...
            <Input label={t('worker.advanceReason')} value={reason} onChangeText={setReason} multiline numberOfLines={3} className="mb-4" />
