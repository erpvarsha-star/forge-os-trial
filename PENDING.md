...
**Last updated:** 24 Sep 2026 (session 4) — GPS + QR dual check-in: `qr_verified` flag now earns a ★ gate star worth the second 50% of attendance score. Security app opens directly to gate QR (first tab). Worker home screen shows QR star prompt after GPS check-in. `nightly-scoring` split: GPS alone = 50% of attendance weight, GPS + gate QR = 100%. `shift-reminder` hourly job now also notifies employees who haven't checked in 45–105 min after shift start. QR scan validation tightened to exact match (interim fallback removed). APK build triggers on push. Remaining open: APPS_SCRIPT_URL, supervisor Telegram onboarding (5 missing: Subhash Palve, Shivaji Jaypure, Manoj Wagh, Sunil Saha, Abhimanyu Kakde), per-form tracking. Action for Yash: (1) paste updated `scripts/ALERT.gs` into live Apps Script editor, run `deployShiftTrackingTriggers()`; (2) HR Admin must assign supervisor_id for Cutting/Press/Machine/HT/Electricity/Oil/VMC supervisors so their team shows on the dashboard (data gap, not a code issue).
