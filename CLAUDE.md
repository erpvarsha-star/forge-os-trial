...
| `PATCH_23_shifts_seed_04Sep2026.sql` | Seeds the three production shifts (Shift 1: 08:30-15:30, Shift 2: 15:30-23:30, Shift 3: 23:30-08:30, marked as night shift). Unblocks HR's ability to assign shifts via `hr-admin/shifts.tsx` and `shift-reminder`'s `dailyCheckinReminder` mode, which had no shifts to match against. No dependency blocker — ready to run | ⏳ Ready to run |
